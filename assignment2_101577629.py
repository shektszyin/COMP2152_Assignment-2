...
class NetworkTool:
    ...
    #preventing invalid date when setting values like empty string 
    @property
    def target(self):
...
class PortScanner(NetworkTool):
    ...
    # if there are any network error would crash the program without the try-except, for
    #example scanning an unreachable port could raise an exception and stop. 
    ...
    #   threading can scanned multiple port at the same time,making it faster.
...
if __name__ == "__main__":
    try:
        ...
        if end_port < start_port:
            print("End port must be between 1 and 1024.")
            exit()
...
#i think i will add a feature to filter open port by service name.
# Diagram: See diagram_101577629.png in the repository root